...
namespace Geometry {
    ...
    class Segment {
        ...
        public:
            ...
            Segment(const Point<FloatType>& point1,
            ...
            Segment
            collinear_segments_intersection(const Segment& segment) const {
                Segment segment_copy = segment;
                if (!direction_vector.is_codirectional(
                        segment.direction_vector)) {
                    segment_copy =
                        Segment(segment.end_point, segment.start_point);
                }

                if (is_point_belong(segment_copy.start_point) &&
                    is_point_belong(segment_copy.end_point)) {
                    return Segment(segment_copy.start_point,
                                   segment_copy.end_point);
                }
            ...
            Segment
            intersection_with_point(const Point<FloatType>& point) const {
            ...
            Segment
            not_collinear_segments_intersection(const Segment& segment) const {
                ...
                std::pair<FloatType, FloatType> params_of_intersection =
                    solve_system_3eq_2var(
                        {start_point.get_x() - end_point.get_x(),
                         segment.end_point.get_x() -
                             segment.start_point.get_x(),
                         segment.end_point.get_x() - end_point.get_x()},
                        {start_point.get_y() - end_point.get_y(),
                         segment.end_point.get_y() -
                             segment.start_point.get_y(),
                         segment.end_point.get_y() - end_point.get_y()},
                        {start_point.get_z() - end_point.get_z(),
                         segment.end_point.get_z() -
                             segment.start_point.get_z(),
