...
namespace Geometry
{
    ...
    public:
        ...
        {
            if (point1.is_equal(Point()) && point2.is_equal(Point()) && point3.is_equal(Point()))
            {
                normal = Vector();
                d = NAN;
                return;
        ...
        void print() const
        {
            std::cout << normal.get_x() << "x" << sign(normal.get_y()) << fabs(normal.get_y()) << "y" << sign(normal.get_z()) << fabs(normal.get_z()) << "z" << sign(d) << fabs(d) << "=0" << std::endl;
        }
