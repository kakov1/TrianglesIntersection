...
namespace Geometry {
template <typename FloatType> class Vector {
private:
  FloatType x = NAN;
  FloatType y = NAN;
  FloatType z = NAN;
...
public:
  ...
  Vector(const Point<FloatType> &point1, const Point<FloatType> &point2) {
  ...
  Vector operator/(FloatType number) const { return operator*(1 / number); }

  bool is_equal(const Vector &vector) const {
    return (is_equal_floats(x, vector.x) && is_equal_floats(y, vector.y) &&
  ...
  bool is_codirectional(const Vector &vector) const {
    ...
    return is_equal_floats(length() * vector.length(), scalar_product(vector));
  }

  FloatType triple_product(const Vector &a, const Vector &b) const {
    return x * det_two(a.y, a.z, b.y, b.z) - y * det_two(a.x, a.z, b.x, b.z) +
...
Vector<FloatType> operator*(FloatType number, const Vector<FloatType> &vector) {
