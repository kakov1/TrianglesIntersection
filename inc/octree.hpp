...
template <typename FloatType> struct CubeParams {
  FloatType x_min;
  FloatType y_min;
  FloatType z_min;
  FloatType x_max;
...
template <typename FloatType> class Cube {
private:
...
public:
  ...
  void partition(std::array<Cube, 8> &cubes) const {
    ...
    cubes[1] =
        Cube(Vector<FloatType>(center.get_x(), min.get_y(), min.get_z()),
             Vector<FloatType>(max.get_x(), center.get_y(), center.get_z()));
    cubes[2] =
        Cube(Vector<FloatType>(center.get_x(), min.get_y(), center.get_z()),
             Vector<FloatType>(max.get_x(), center.get_y(), max.get_z()));
    cubes[3] =
        Cube(Vector<FloatType>(min.get_x(), min.get_y(), center.get_z()),
             Vector<FloatType>(center.get_x(), center.get_y(), max.get_z()));
    cubes[4] =
        Cube(Vector<FloatType>(min.get_x(), center.get_y(), min.get_z()),
             Vector<FloatType>(center.get_x(), max.get_y(), center.get_z()));
    cubes[5] =
        Cube(Vector<FloatType>(center.get_x(), center.get_y(), min.get_z()),
             Vector<FloatType>(max.get_x(), max.get_y(), center.get_z()));
    cubes[6] = Cube(center, max);
    cubes[7] =
        Cube(Vector<FloatType>(min.get_x(), center.get_y(), center.get_z()),
  ...
  bool is_part_of_triangle_in_cube(const Triangle<FloatType> &triangle) const {
    return is_point_in_cube(triangle.get_a()) ||
           is_point_in_cube(triangle.get_b()) ||
           is_point_in_cube(triangle.get_c());
  }
...
template <typename FloatType> class Octree {
private:
  class Node;

  using triangles_list = std::list<std::pair<Triangle<FloatType>, size_t>>;
  ...
  private:
    ...
    void build_tree() {
      ...
      Vector<FloatType> cube_params = region_.get_max() - region_.get_min();
    ...
    void
    get_intersections_with_children(std::set<size_t> &result,
    ...
    void get_triangles(triangles_list &triangles) const {
      triangles.insert(triangles.begin(), triangles_.begin(), triangles_.end());
...
public:
  Octree(const triangles_list &triangles, const CubeParams<FloatType> &region) {
