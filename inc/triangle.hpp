...
namespace Geometry {
template <typename FloatType> class Triangle {
private:
  ...
  bool is_in_same_half_plane(const Plane<FloatType> &plane) const {
    return (plane.is_point_over_plane(a) && plane.is_point_over_plane(b) &&
            plane.is_point_over_plane(c)) ||
           (plane.is_point_under_plane(a) && plane.is_point_under_plane(b) &&
  ...
  void compute_interval(Point<FloatType> point, FloatType &min,
                        FloatType &max) const {
    min = max = Vector<FloatType>({0, 0, 0}, get_vertice(0))
  ...
  bool is_intersect_in_same_plane(const Triangle &triangle) const {
    ...
    for (int index1 = 0, index2 = 2; index1 < 3; index2 = index1, index1++) {
      Vector<FloatType> side(get_vertice(index2), get_vertice(index1));
      Vector<FloatType> perp_side = plane.find_perp_in_plane(side);

      compute_interval(Point<FloatType>(perp_side.get_x(), perp_side.get_y(),
                                        perp_side.get_z()),
                       min1, max1);
      triangle.compute_interval(Point<FloatType>(perp_side.get_x(),
                                                 perp_side.get_y(),
                                                 perp_side.get_z()),
                                min2, max2);
    ...
    for (int index1 = 0, index2 = 2; index1 < 3; index2 = index1, index1++) {
      ...
      Vector<FloatType> perp_side = triangle.plane.find_perp_in_plane(side);
  ...
  Segment<FloatType>
  intersection_line_in_same_plane(const Line<FloatType> &line) const {
    ...
    if (!Segment<FloatType>(a, b).is_point_belong(intersection_point_ab)) {
      intersection_point_ab = Point<FloatType>();
    }
    if (!Segment<FloatType>(b, c).is_point_belong(intersection_point_bc)) {
      intersection_point_bc = Point<FloatType>();
    }
    if (!Segment<FloatType>(a, c).is_point_belong(intersection_point_ac)) {
  ...
  bool is_intersect_for_segment(const Triangle &triangle) const {
    if (triangle.is_point()) {
      return Segment<FloatType>(a, c).is_point_belong(triangle.a);
    } else if (triangle.is_segment()) {
...
public:
  ...
  Triangle(const Point<FloatType> &point1, const Point<FloatType> &point2,
    ...
    if (is_degenerate()) {
      plane = {Point<FloatType>(), Point<FloatType>(), Point<FloatType>()};
      return;
    }
    plane = Plane(a, b, c);
  }
  ...
  bool is_intersect(const Triangle &triangle) const {
    if (is_point()) {
      ...
      if (triangle.is_point()) {
        ...
        return is_intersect_segment(Segment<FloatType>(triangle.a, triangle.c));
      }
    }
    if (plane.is_collinear(triangle.plane)) {
      ...
      if (triangle.is_in_same_half_plane(triangle.plane)) {
        ...
        Line<FloatType> intersection_line = plane.intersection(triangle.plane);
  ...
  bool is_intersect_segment(const Segment<FloatType> &segment) const {
    Vector<FloatType> e1 = Vector<FloatType>(a, b),
                      e2 = Vector<FloatType>(a, c);
    Vector<FloatType> p = segment.get_direction_vector().vector_product(e2);
    ...
    tmp = 1 / tmp;
    Vector<FloatType> s = Vector<FloatType>(a, segment.get_start_point());
    ...
    return segment.is_point_belong(Point<FloatType>(
        intersection.get_x(), intersection.get_y(), intersection.get_z()));
  }
};
} // namespace Geometry