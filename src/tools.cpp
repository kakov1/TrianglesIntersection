...
std::pair<double, double> Geometry::solve_system_2eq_2var(const Geometry::Vector& row1,
                                                const Geometry::Vector& row2) {
    double main_det = Geometry::det_two(row1.get_x(), row1.get_y(),
                              row2.get_x(), row2.get_y());
    double x_det = Geometry::det_two(row1.get_z(), row1.get_y(),
                           row2.get_z(), row2.get_y());
    double y_det = Geometry::det_two(row1.get_x(), row1.get_z(),
                           row2.get_x(), row2.get_z());
    
    if (Geometry::is_equal_doubles(main_det, 0)) {
        if (Geometry::is_equal_doubles(x_det, 0) && Geometry::is_equal_doubles(y_det, 0)) {
...
std::pair<double, double> Geometry::solve_system_3eq_2var(const Geometry::Vector& row1,
                                                const Geometry::Vector& row2,
                                                const Geometry::Vector& row3) {
    std::pair<double, double> solution12 = Geometry::solve_system_2eq_2var(row1, row2);
    std::pair<double, double> solution23 = Geometry::solve_system_2eq_2var(row2, row3);
    std::pair<double, double> solution13 = Geometry::solve_system_2eq_2var(row1, row3);

    if (is_inf_solution(solution12) &&
        is_inf_solution(solution23) &&
        is_inf_solution(solution13)) return Geometry::INFINITY_SOLUTION;
